...
@Setter //Lombok annotation: Automatically generates setter methods for all private fields at compile time
    ...
    */
    ...
    /*
        *Relationship Mapping: One owner can own multiple properties.
        - mappedBy = "owner": Indicates that the 'property' entity owns the foreign key relationship.
          The field named 'owner' in the Property class manages the database join column

        - cascade = CascadeType.ALL: Performs all persistence operations (PERSIST, REMOVE REFRESH, MARGE)
          transitively to child entities.  Eg. Deleting an owner automatically deletes all their properties.

        - @JsonBackReference: Jackson annotation to prevent infinite recursion during  JSON serialization.
          It makes this field as the 'child/back' side of the relationship so jackson ignors it when converting an Owner instance to JSON.
    */
